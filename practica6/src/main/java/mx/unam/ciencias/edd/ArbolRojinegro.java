    ...
    private void rebalanceaElimina(VerticeRojinegro v) {
	    ...
	    if ( padre == null ) { 
		    ...
		    if ( isRed(hermano) ) {
			    ...
			    // Caso 3
			    if ( !isRed(hijoIzqHermano) && !isRed(hijoDerHermano) && !isRed(hermano) && !isRed(padre) ) {
			    ...
			    if ( isLeftSon(v) && isRed(hijoIzqHermano) && !isRed(hijoDerHermano) ) {				   
				    hermano.color = Color.ROJO;
			    ...
			    if ( !isLeftSon(v) && isRed(hijoDerHermano) && !isRed(hijoIzqHermano) ) {
				    hermano.color = Color.ROJO;
